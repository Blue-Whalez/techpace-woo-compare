	...
	$.wc_products_compare_frontend = {
		...
		popupCompare : function(e){
			...
			function checkAvailable (el){
				let count_cookies= $.wc_products_compare_frontend.getComparedProducts().length - 1;
			...
			if(popup.length && popup_trigger.length){
				popup_trigger.on('click', function(){
					...
					if(!popup.hasClass('active')){
						//add product on UI
						//check số lượng
						let compare_list= $.wc_products_compare_frontend.getComparedProducts();
						...
						if(count_items < 3){
							...
							if(compare_list && compare_list.includes(this_product.id.toString())){
								popup_mess.text('Bạn đã chọn sản phẩm này rồi, vui lòng chọn sản phẩm khác!');
							}
							else{
								...
								//add to cookie
								$.wc_products_compare_frontend.setComparedProducts(this_product.id);
								//add to widget
								$.wc_products_compare_frontend.widgetAddProduct(this_product.id);

								console.log('added js', compare_list);
							}
						}
						else{
							popup_mess.text('Vui lòng xóa bớt sản phẩm để tiếp tục so sánh!');
						}
						//open
